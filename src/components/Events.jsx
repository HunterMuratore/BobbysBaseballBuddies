      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="flex flex-col gap-5">
          {events.map(event => (
            <div
              key={event.id}
              className="flex gap-6 items-start border border-tan rounded-2xl p-6 md:p-8 hover:border-navy/20 hover:shadow-sm transition-all duration-200 bg-white group"
            >
              {/* Date block */}
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-navy/60 leading-relaxed">{event.description}</p>
              </div>
            </div>
          ))}
        </div>
